...
export default function UsersControls({
  ...
  const push = useCallback(
    (updates: Record<string, string>) => {
      ...
      // sempre volta pra página 1 ao mudar filtros (exceto quando muda page)
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', marginBottom: '1rem' }}>
      ...
      {totalPages > 1 && (
        <div style={{ display: 'flex', gap: '0.375rem', alignItems: 'center', flexWrap: 'wrap' }}>
          ...
          {/* Páginas numéricas — exibe até 5 ao redor da atual */}
          {Array.from({ length: totalPages }, (_, i) => i + 1)
            ...
            .map((p, i) =>
              p === '...'
                ? <span key={`ellipsis-${i}`} style={{ padding: '0 0.25rem', color: 'var(--color-text-muted)', fontSize: '0.8rem' }}>…</span>
                : (
                  <button
                    key={p}
          ...
          <button
            className="btn btn-ghost btn-sm"
            disabled={page >= totalPages || isPending}
            onClick={() => push({ page: String(totalPages) })}
          >»</button>
        </div>
      )}
