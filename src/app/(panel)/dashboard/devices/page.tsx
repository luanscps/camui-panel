...
function isOnline(dateStr: string | null): boolean {
  ...
  return Date.now() - new Date(dateStr).getTime() < 5 * 60 * 1000 // 5 min
}
...
export default async function DevicesPage() {
  ...
  const { data: activations } = license
    ? await supabase
        ...
        .select('id, device_name, device_brand, device_model, device_hardware, android_version, sdk_int, android_id, app_version, fingerprint, activated_at, last_seen')
              ...
              <div key={device.id} className="card" style={{ padding: 0, overflow: 'hidden' }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', flexWrap: 'wrap' }}>
                      ...
                      <span style={{
                        ...
                        background: online ? 'var(--color-success-bg, rgba(67,122,34,0.08))' : 'var(--color-surface-offset)',
                      ...
                      {idx === 0 && !online && (
                        <span style={{ fontSize: '0.6875rem', color: 'var(--color-text-faint)' }}>último ativo</span>
                      )}
                    </div>
                ...
                <div style={{
                  ...
                  {[
                    ...
                    {
                      icon: '⚙️',
                      label: 'Modelo / Hardware',
                      value: device.device_model ?? device.device_hardware ?? '—',
                    },
                    {
                      icon: '🤖',
                      label: 'Android',
                    ...
                    },
                    {
                      ...
                      value: device.android_id
                        ...
                        : '—',
                      mono: true,
                      title: device.android_id ?? undefined,
                    },
                    {
                      icon: '🧬',
                      label: 'Fingerprint',
                      value: device.fingerprint
                        ? `${device.fingerprint.replace(/\|/g, ' · ').slice(0, 28)}…`
                        : '—',
                      mono: true,
                    ...
                    },
                    {
                      ...
                      value: device.activated_at
                        ? new Date(device.activated_at).toLocaleDateString('pt-BR', { day: '2-digit', month: 'short', year: 'numeric' })
                        : '—',
                    },
                    {
                      icon: '🕐',
